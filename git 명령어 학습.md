...
- branch란?
  기능 변경을 하고싶을 때 생성 및 사용. 코드를 개발하면서 초보일 수록 코드가 엉망진창 주석으로 뒤섞인 코드를 짜는데 이를 스파게티 코드라고 합니다. 이런 상황을 막기 위해서 브랜치를 사용합니다. 

```
git checkout branch_name //branch 변경, 변경하기전에 commit push를 진행해야 한다.
git branch -a //모든 branch 출력
git branch "브랜치 이름" //create new branch
```

기본적으로 master 브랜치로 작업을 진행하게 된다. 브랜치의 경우, 각기 다른 작업을 할 때 용이합니다. 가상의 작업 환경이라 보면 좋습니다. 

- 깃에서는 **한번에 하나**의 브랜치에서만 작업이 가능합니다. 이를 HEAD 브랜치라고 부릅니다. 현재 작업 중인 브랜치를 헤드 브랜치라고 합니다. 
...
커밋과 커밋메시지를 입력합니다. comiit은 스테이지에 올라온 파일들을 가지고 내 컴퓨터에 저장합니다. - 이 말이 이해가 정확히 안가네.
...
한 브랜치의 내용을 다른 브랜치와 합칠 때 사용한다. 현재 브랜치를 HEAD 브랜치라고 하는데, 여기에 다른 브랜치를 병합하면 다른 브랜치의 내용이 HEAD에 반영되게 됩니다. 이 때 다양한 상황에 접하게 됩니다.

- HEAD 브랜치에 변경 사항이 없을 경우
  브랜치의 참조만 갱신되는 상황?
  fast_forward
...
- 가지가 생겨난 경우
  ...
  - 여러 브랜치를 동시에 작업하면서 병합을 시도한 경우

  여러 브랜치에서 작업하면 conflict 나는 건 당연하지. 



> 브랜치 관련해서 작업은 이해가 잘 가지 않지만 일단은 적었는데, 나중에 꼭 다시 봐야할 것 같습니다. 
